...
现在我们将文字输入框与其面板关联起来。我们通过引入自动完成面板的实例到一个本地模块变量（我们定义其为“auto”），并绑定这个变量到输入框的mdAutocomplete属性。
...
现在自动完成面板可以通过焦点切换，也可以选择备选项。但如果我们想在输入的同时过滤备选项，需要添加一个自定义过滤器。
...
\*我们可能考虑在页面中添加文本指导来解释过滤器规则。假设你使用了一个不限制从头匹配的非标准的过滤器，这种对使用者来说特别有帮助。
